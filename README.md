...
This repo holds the location-data work for our Topic Two model. The main output is a WGS84 coordinate table with one row per unique SSEN substation location.

## Included files

- `locations.py` downloads the official SSEN substation source, removes repeated physical locations and converts British National Grid coordinates to latitude/longitude.
...
The 6 km value in the district file is a working midpoint within Huawei's stated 3–9 km weather radius. It is not a Huawei-prescribed value. The individual substation CSV contains point coordinates and does not use that radius.
